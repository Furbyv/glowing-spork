...
import { FullWozObjectReply, FullWozObjectSaveReply, WozObjectRequestById } from 'src/app/protos/wozobject.pb';
...
export class GetFullWozObjectService {
  ...
  private saveWozObjectRequest$$: Subject<FullWozObjectReply> = new Subject<FullWozObjectReply>();
  ...
  fullWozObject$: Observable<AsyncState<FullWozObjectReply>> = this.wozObjectRequest$$.pipe(
  ...
  saveState$: Observable<AsyncState<FullWozObjectSaveReply>> = combineLatest([this.saveWozObjectRequest$$]).pipe(
